...
class ControllerExtensionPaymentPayzenMulti extends ControllerExtensionPaymentPayzen
{
    ...
    protected function validate()
    {
        ...
        if (isset($data['payzen_multi_first'])
            ...
            $this->error = sprintf($this->language->get('error_payzen_multi_validation'), $this->language->get('entry_payzen_first'));

        } elseif (! isset($data['payzen_multi_count'])
            || ! preg_match('#^[1-9]\d*$#', $data['payzen_multi_count'])) {
            $this->error = sprintf($this->language->get('error_payzen_multi_validation'), $this->language->get('entry_payzen_count'));

        } elseif (! isset($data['payzen_multi_period'])
            || ! preg_match('#^[1-9]\d*$#', $data['payzen_multi_period'])) {
            $this->error = sprintf($this->language->get('error_payzen_multi_validation'), $this->language->get('entry_payzen_period'));
        }
