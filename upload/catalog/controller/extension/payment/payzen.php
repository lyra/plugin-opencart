...
class ControllerExtensionPaymentPayzen extends Controller
{
    ...
    public function index()
    {
        ...
        $this->writeLog('Data to send to payment gateway:' . print_r($payzenRequest->getRequestFieldsArray(true /* To hide sensitive data. */), true), 'DEBUG');
    ...
    protected function getPayzenRequest()
    {
        ...
        if ($orderInfo) {
            ...
            $info['order_info'] = 'session_id=' . session_id() . '&session_key=' . $this->session->getId();
            ...
            $configParams = array(
                ...
                'available_languages', 'capture_delay', 'validation_mode', 'payment_cards', 'redirect_enabled',
    ...
    public function callback()
    {
        ...
        $this->writeLog('Data received from payment gateway: ' . print_r($data, true), 'DEBUG');
        ...
        if ($this->isOrderPending($orderInfo)) {
            ...
            if ($payzenResponse->isAcceptedPayment()) {
                $this->writeLog("Payment for order #$orderId has been confirmed by notification URL. New order status is " . $this->config->get($this->name . '_order_status_success') . '.');
                ...
                $sessionId = substr($parts[0], strlen('session_id='));
                $sessionKey = substr($parts[1], strlen('session_key='));

                session_destroy();
                session_id($sessionId);
                session_start();

                $this->session->start('default', $sessionKey);
            ...
            if ($this->isOrderSuccess($orderInfo) && $payzenResponse->isAcceptedPayment()) {
                ...
                $this->writeLog("Payment notification confirmed. Payment error or cancelation for order #$orderId.");
                $this->writeLog('End server response processing.');
    ...
    public function process()
    {
        ...
        $this->writeLog('Data received from payment gateway: ' . print_r($data, true));
    ...
    {
        $this->writeLog("order_status_id " . gettype($orderInfo['order_status_id']) . ' / config ' . gettype($this->config->get($this->prefix . $this->name . '_order_status_success')));
