...
class ControllerPaymentPayzen extends Controller {
	...
	protected function getPayzenRequest() {
		...
		if ($orderInfo) {
			...
			$info['contrib'] = 'OpenCart2.0-2.2_2.1.0/'.VERSION;
	...
	public function callback() {
		...
		if(!$payzenResponse->isAuthentified()) {
			$this->writeLog('Invalid signature. It may be a hacking attempt.', 'ERROR');
		...
		if($this->isOrderPending($orderInfo)) {
			...
			if ($payzenResponse->isAcceptedPayment()) {
				$this->writeLog('Payment accepted. New order status is '.$this->config->get($this->name.'_order_status_success').'.');
	...
	private function renderResponse($errorMessage=false, $urlCheckWarn=false, $data=array()) {
		...
		if($this->config->get($this->name.'_ctx_mode') == 'TEST') {
			$data['text_payzen_pass_to_prod'] = $this->language->get('text_payzen_pass_to_prod_info');
			$data['text_payzen_pass_to_prod'] .= '<a href="https://secure.payzen.eu/html/faq/prod" target="_blank">https://secure.payzen.eu/html/faq/prod</a>';
		}
