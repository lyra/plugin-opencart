...
if (! class_exists('PayzenRequest', false)) {
	...
	class PayzenRequest
	{
		...
		{
			// initialize encoding
			$this->encoding = in_array(strtoupper($encoding), PayzenApi::$SUPPORTED_ENCODINGS) ? strtoupper($encoding) : 'UTF-8';
			...
			$regex_trans_date = '#^\d{4}(1[0-2]|0[1-9])(3[01]|[1-2]\d|0[1-9])(2[0-3]|[0-1]\d)([0-5]\d){2}$#u'; // AAAAMMJJhhmmss
			...
			$this->addField('vads_ship_to_speed', 'Speed of the shipping method', '#^STANDARD|EXPRESS$#u', false, 8);
			...
			$this->addField('vads_ship_to_type', 'Type of the shipping method', '#^RECLAIM_IN_SHOP|RELAY_POINT|RECLAIM_IN_STATION|PACKAGE_DELIVERY_COMPANY|ETICKET$#u', false, 24);
			...
			$this->addField('vads_theme_config', 'Theme', $ans255);
		...
		public function setMultiPayment($total_in_cents = null, $first_in_cents = null, $count = 3, $period = 30)
		{
			...
			if (is_numeric($count) && $count > 1 && is_numeric($period) && $period > 0) {
				...
				// check parameters
				if (is_numeric($total_in_cents) && $total_in_cents > $first_in_cents && $total_in_cents > 0 && is_numeric($first_in_cents) && $first_in_cents > 0) {
		 ...
		 * @return string
		 */
		public function getRequestHtmlForm($form_add = '', $input_type = 'hidden', $input_add = '', $btn_type = 'submit', $btn_value = 'Pay', $btn_add = '', $escape = true)
		{
			$html = '';
